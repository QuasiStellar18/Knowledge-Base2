...
function bindEvents() {
    ...
    // Prevent clicks inside embed cards from triggering outer navigation.
    // Use capture phase so we intercept before other handlers.
    document.addEventListener("click", (e) => {
        try {
            const wrap = e.target.closest && e.target.closest(".embedWrap");
            if (!wrap) return;
            // Allow the explicit Open button to work
            if (e.target.closest && e.target.closest(".embedOpen")) return;
            // Prevent default navigation and stop propagation so media controls work
    ...
    if (embed) {
        ...
        try {
            ...
            if (tag === "video" || tag === "audio") {
                ...
                embed.addEventListener("click", handler, { passive: false });
                embed.addEventListener("play", handler, { passive: false });
                embed.addEventListener("pause", handler, { passive: false });
            }

            // For iframes, add a click handler on the iframe element itself.
            if (tag === "iframe") {
...
function createEmbed(url) {
    ...
    // Redgifs embeds: try to load a direct MP4 via the public API and
    // replace with a native <video> player so pause/play/fullscreen work.
    try {
        const parsed = new URL(url);
        const host = parsed.hostname.toLowerCase();
        if (host.includes("redgifs.com") || host.includes("redgif.com") || host === "r.redgifs.com") {
            const parts = parsed.pathname.split("/").filter(Boolean);
            const id = parts.pop();
            if (id) {
                const placeholder = document.createElement("div");
                placeholder.className = "embed embedLoading";

                // Async load MP4 via Redgifs API; fall back to iframe on error.
                (async () => {
                    try {
                        ...
                        // Search response for an mp4 URL in known places.
                        let mp4 = null;
                        const tryPaths = [
                            data?.gif?.urls?.hd,
                            data?.gif?.urls?.sd,
                        ...
                        for (const p of tryPaths) {
                            if (typeof p === "string" && p.match(/https?:\/\/.*\.mp4(\?.*)?$/i)) {
                                mp4 = p;
                                break;
                            }
                        }

                        // Fallback: try to find any mp4 link deep in the JSON
                        if (!mp4) {
                            const jsonText = JSON.stringify(data || {});
                            const m = jsonText.match(/https?:\\/\\/[^\"]+?\\.mp4(\\?[^\"]*)?/i);
                            if (m) mp4 = m[0].replace(/\\/g, "");
                        }
                        ...
                        if (mp4) {
                            ...
                            // Ensure media interactions are isolated
                            const handler = (e) => {
                                e.preventDefault && e.preventDefault();
                                e.stopImmediatePropagation && e.stopImmediatePropagation();
                                e.stopPropagation && e.stopPropagation();
                            ...
                            video.addEventListener("click", handler, { passive: false });
                            video.addEventListener("play", handler, { passive: false });
                            video.addEventListener("pause", handler, { passive: false });

                            placeholder.replaceWith(video);
                            return;
                        }
                    } catch (err) {
                        // ignore and fall back to iframe below
                    }

                    // fallback iframe if API or parsing fails
                    const iframe = document.createElement("iframe");
                    iframe.className = "embed";
